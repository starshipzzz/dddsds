...
async def manage_pending_reviews(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Échapper les underscores dans l'username pour le Markdown
    username = review.get('username', '').replace('_', '\\_')
    # Afficher le @ Telegram seulement pour les admins avec l'username échappé
    admin_info = f"📱 Telegram: @{username}\n" if str(update.effective_user.id) in ADMIN_IDS else ""
...
async def post_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bloquer si l'utilisateur a déjà un avis
    if has_review or has_pending:
        keyboard = [[InlineKeyboardButton("🔙 Retour", callback_data="show_reviews")]]
        await query.edit_message_text(
    ...
    # Si l'utilisateur n'a pas encore posté d'avis, continuer normalement
